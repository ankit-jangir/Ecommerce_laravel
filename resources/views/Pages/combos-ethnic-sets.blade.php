...
@section('title', 'Ethnic Sets - AURA KURTIS')

@section('content')

@php
// Get products from MockData - specifically ethnic sets
$allProducts = \App\Helpers\MockData::getHomepageProducts();
$ethnicSetsProducts = $allProducts['combos_ethnic_sets'] ?? [];
$bestsellers = $allProducts['bestsellers'] ?? [];

// If no specific ethnic sets, use all products and filter
if (empty($ethnicSetsProducts)) {
    $ethnicSetsProducts = collect($allProducts)->flatten(1)->toArray();
}

// Add hover images and assign to ethnic sets
$productsWithHover = [];
foreach ($ethnicSetsProducts as $index => $product) {
    $nextIndex = ($index + 1) % count($ethnicSetsProducts);
    $product['hover_image'] = $ethnicSetsProducts[$nextIndex]['image'] ?? $product['image'];
    ...
    $product['subcategory'] = 'Ethnic Sets';
    ...
    <div class="relative container mx-auto px-4 sm:px-6 h-full flex items-center justify-center text-center">
        <div class="text-white max-w-3xl">
            ...
            <p class="mt-4 text-sm sm:text-base text-gray-200 max-w-2xl mx-auto">
                Discover elegant ethnic sets crafted for modern women
            </p>
        </div>
    </div>
</section>
