...
->filter(function ($product) use ($selectedCategories, $maxPrice) {
...
$perPage = 6;
$page = request()->get('page', 1);
$totalPages = ceil($filteredProducts->count() / $perPage);
        ...
        <aside class="lg:col-span-1 space-y-8 top-28 h-fit">
            <form method="GET">
                ...
                <div>
                    ...
                    <label class="flex items-center gap-3 text-sm cursor-pointer mb-2">
                        <input type="checkbox" name="category[]" value="{{ $cat }}"
                            {{ in_array($cat, request()->get('category', [])) ? 'checked' : '' }}
                            class="w-4 h-4 rounded text-black focus:ring-black">
                        {{ $cat }}
                    </label>
                    @endforeach
                </div>
                ...
                <div>
                    ...
                    <input type="range" name="price" min="500" max="5000" value="{{ request()->get('price', 5000) }}"
                        class="w-full accent-black"
                        oninput="document.getElementById('priceValue').innerText = this.value">

                    <div class="flex justify-between text-xs text-gray-500 mt-1">
                        <span>₹500</span>
                        <span>₹<span id="priceValue">{{ request()->get('price', 5000) }}</span></span>
                        <span>₹5000</span>
                    </div>
                </div>
        ...
        <div class="lg:col-span-3">
                ...
                <div class="group">
                    ...
                    <div class="relative overflow-hidden rounded-2xl">
                        ...
                        group-hover:opacity-100 group-hover:translate-x-0 transition">
                            ...
                            <!-- VIEW -->
                            <a href="{{ route('product.detail', $product['slug']) }}" class="w-10 h-10 bg-white rounded-full shadow flex items-center justify-center
          hover:bg-black hover:text-white transition">
            ...
            <div class="flex justify-center mt-14 gap-2">
                @for ($p = 1; $p <= $totalPages; $p++) <a href="{{ request()->fullUrlWithQuery(['page' => $p]) }}"
