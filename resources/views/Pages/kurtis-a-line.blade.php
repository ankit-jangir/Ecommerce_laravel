...
@php
// Get products from MockData - specifically a-line kurtis
$allProducts = \App\Helpers\MockData::getHomepageProducts();
$alineProducts = $allProducts['kurtis_a_line'] ?? [];
$bestsellers = $allProducts['bestsellers'] ?? [];

// If no specific a-line, use all products and filter
if (empty($alineProducts)) {
    $alineProducts = collect($allProducts)->flatten(1)->toArray();
}

// Add hover images and assign to a-line
$productsWithHover = [];
foreach ($alineProducts as $index => $product) {
    $nextIndex = ($index + 1) % count($alineProducts);
    $product['hover_image'] = $alineProducts[$nextIndex]['image'] ?? $product['image'];
    $product['color'] = ['Red', 'Blue', 'Black', 'Pink', 'Green', 'Yellow', 'White', 'Orange'][$index % 8] ?? 'Red';
    $product['size'] = ['S', 'M', 'L', 'XL', 'XXL'][$index % 5] ?? 'M';
    $product['category'] = 'A-Line';
    $product['subcategory'] = 'A-Line';
    ...
    <div class="relative container mx-auto px-4 sm:px-6 h-full flex items-center justify-center text-center">
        <div class="text-white max-w-3xl">
            ...
            <p class="mt-4 text-sm sm:text-base text-gray-200 max-w-2xl mx-auto">
                Discover elegant A-Line kurtis crafted for modern women
            </p>
        </div>
    </div>
</section>
