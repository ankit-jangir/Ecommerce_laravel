...
@section('title', 'Women Tops - AURA KURTIS')

@section('content')

@php
// Get products from MockData - specifically women tops
$allProducts = \App\Helpers\MockData::getHomepageProducts();
$topsProducts = $allProducts['women_tops'] ?? [];
$bestsellers = $allProducts['bestsellers'] ?? [];

// If no specific tops, use all products and filter
if (empty($topsProducts)) {
    $topsProducts = collect($allProducts)->flatten(1)->toArray();
}

// Add hover images and assign to tops
$productsWithHover = [];
foreach ($topsProducts as $index => $product) {
    $nextIndex = ($index + 1) % count($topsProducts);
    $product['hover_image'] = $topsProducts[$nextIndex]['image'] ?? $product['image'];
    ...
    <div class="relative container mx-auto px-4 sm:px-6 h-full flex items-center justify-center text-center">
        <div class="text-white max-w-3xl">
            ...
            <p class="mt-4 text-sm sm:text-base text-gray-200 max-w-2xl mx-auto">
                Discover elegant tops crafted for modern women
            </p>
        </div>
    </div>
</section>
