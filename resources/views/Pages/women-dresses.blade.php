...
@php
// Get products from MockData - specifically women dresses
$allProducts = \App\Helpers\MockData::getHomepageProducts();
$dressesProducts = $allProducts['women_dresses'] ?? [];
$bestsellers = $allProducts['bestsellers'] ?? [];

// If no specific dresses, use all products and filter
if (empty($dressesProducts)) {
    $dressesProducts = collect($allProducts)->flatten(1)->toArray();
}

// Add hover images and assign to dresses
$productsWithHover = [];
foreach ($dressesProducts as $index => $product) {
    $nextIndex = ($index + 1) % count($dressesProducts);
    $product['hover_image'] = $dressesProducts[$nextIndex]['image'] ?? $product['image'];
    ...
    <div class="relative container mx-auto px-4 sm:px-6 h-full flex items-center justify-center text-center">
        <div class="text-white max-w-3xl">
            ...
            <p class="mt-4 text-sm sm:text-base text-gray-200 max-w-2xl mx-auto">
                Discover elegant dresses crafted for modern women
            </p>
        </div>
    </div>
</section>
