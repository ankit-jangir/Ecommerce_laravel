...
@php
// Get products from MockData - specifically printed kurtis
$allProducts = \App\Helpers\MockData::getHomepageProducts();
$printedProducts = $allProducts['kurtis_printed'] ?? [];
$bestsellers = $allProducts['bestsellers'] ?? [];

// If no specific printed, use all products and filter
if (empty($printedProducts)) {
    $printedProducts = collect($allProducts)->flatten(1)->toArray();
}

// Add hover images and assign to printed
$productsWithHover = [];
foreach ($printedProducts as $index => $product) {
    $nextIndex = ($index + 1) % count($printedProducts);
    $product['hover_image'] = $printedProducts[$nextIndex]['image'] ?? $product['image'];
    ...
    <div class="relative container mx-auto px-4 sm:px-6 h-full flex items-center justify-center text-center">
        <div class="text-white max-w-3xl">
            ...
            <p class="mt-4 text-sm sm:text-base text-gray-200 max-w-2xl mx-auto">
                Discover elegant printed kurtis crafted for modern women
            </p>
        </div>
    </div>
</section>
