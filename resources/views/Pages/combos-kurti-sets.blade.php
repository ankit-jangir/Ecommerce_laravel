...
@section('title', 'Kurti Sets - AURA KURTIS')

@section('content')

@php
// Get products from MockData - specifically kurti sets
$allProducts = \App\Helpers\MockData::getHomepageProducts();
$kurtiSetsProducts = $allProducts['combos_kurti_sets'] ?? [];
$bestsellers = $allProducts['bestsellers'] ?? [];

// If no specific kurti sets, use all products and filter
if (empty($kurtiSetsProducts)) {
    $kurtiSetsProducts = collect($allProducts)->flatten(1)->toArray();
}

// Add hover images and assign to kurti sets
$productsWithHover = [];
foreach ($kurtiSetsProducts as $index => $product) {
    $nextIndex = ($index + 1) % count($kurtiSetsProducts);
    $product['hover_image'] = $kurtiSetsProducts[$nextIndex]['image'] ?? $product['image'];
    $product['color'] = ['Red', 'Blue', 'Black', 'Pink', 'Green', 'Yellow', 'White', 'Orange'][$index % 8] ?? 'Red';
    $product['size'] = ['S', 'M', 'L', 'XL', 'XXL'][$index % 5] ?? 'M';
    $product['category'] = 'Kurti Sets';
    $product['subcategory'] = 'Kurti Sets';
    ...
    <div class="relative container mx-auto px-4 sm:px-6 h-full flex items-center justify-center text-center">
        <div class="text-white max-w-3xl">
            ...
            <p class="mt-4 text-sm sm:text-base text-gray-200 max-w-2xl mx-auto">
                Discover elegant kurti sets crafted for modern women
            </p>
        </div>
    </div>
</section>
