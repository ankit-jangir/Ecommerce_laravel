...
@section('title', 'Kurtis Collection - AURA KURTIS')

@section('content')

@php
// Get all products from MockData - specifically kurtis
$allProducts = \App\Helpers\MockData::getHomepageProducts();
$kurtiProducts = $allProducts['kurtis'] ?? [];
$bestsellers = $allProducts['bestsellers'] ?? [];

// If no kurti products, use all products
if (empty($kurtiProducts)) {
    $kurtiProducts = collect($allProducts)->flatten(1)->toArray();
}

// Kurtis-specific subcategories
$kurtiSubcategories = ['Anarkali', 'Straight Cut', 'A-Line', 'Printed'];

// Add hover images and assign to kurti subcategories
$productsWithHover = [];
foreach ($kurtiProducts as $index => $product) {
    $nextIndex = ($index + 1) % count($kurtiProducts);
    $product['hover_image'] = $kurtiProducts[$nextIndex]['image'] ?? $product['image'];
    ...
    <div class="relative container mx-auto px-4 sm:px-6 h-full flex items-center justify-center text-center">
        <div class="text-white max-w-3xl">
            ...
            <p class="mt-4 text-sm sm:text-base text-gray-200 max-w-2xl mx-auto">
                Discover elegant kurtis in various styles - Anarkali, Straight Cut, A-Line and more
            </p>
        </div>
    </div>
</section>
                        ...
                        <div class="space-y-1">
                            <details class="group" open>
                                ...
                                <div class="pl-2 mt-1 sm:mt-2 space-y-1 sm:space-y-2">
                                    @foreach($kurtiSubcategories as $subcat)
