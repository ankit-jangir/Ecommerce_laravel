...
@section('title', 'Men Collection - AURA KURTIS')

@section('content')

@php
// Get all products from MockData - specifically men's products
$allProducts = \App\Helpers\MockData::getHomepageProducts();
$menProducts = $allProducts['men'] ?? [];
$bestsellers = $allProducts['bestsellers'] ?? [];

// If no men products, use all products
if (empty($menProducts)) {
    $menProducts = collect($allProducts)->flatten(1)->toArray();
}

// Men-specific subcategories
$menSubcategories = ['Winterwear', 'Shirts', 'Kurtas & Jackets', 'T-Shirts', 'Trousers', 'Gifts for him', 'New Arrivals', 'Bestsellers'];

// Add hover images and assign to men subcategories
$productsWithHover = [];
foreach ($menProducts as $index => $product) {
    $nextIndex = ($index + 1) % count($menProducts);
    $product['hover_image'] = $menProducts[$nextIndex]['image'] ?? $product['image'];
    ...
    $productCategory = $product['category'] ?? 'Shirts';
...
$colors = ['Red', 'Blue', 'Black', 'Pink', 'Green', 'Yellow', 'White', 'Orange', 'Navy', 'Gray'];
$sizes = ['S', 'M', 'L', 'XL', 'XXL'];
@endphp
    ...
    <div class="relative container mx-auto px-4 sm:px-6 h-full flex items-center justify-center text-center">
        <div class="text-white max-w-3xl">
            ...
            <p class="mt-4 text-sm sm:text-base text-gray-200 max-w-2xl mx-auto">
                Discover premium shirts, kurtas, t-shirts and trousers for modern men
            </p>
        </div>
    </div>
</section>
                        ...
                        <div class="space-y-1">
                            <details class="group" open>
                                ...
                                <div class="pl-2 mt-1 sm:mt-2 space-y-1 sm:space-y-2">
                                    @foreach($menSubcategories as $subcat)
                        ...
                        <div class="flex flex-wrap gap-1.5 sm:gap-2">
                            ...
                            <label class="cursor-pointer">
                                ...
                                <div class="w-6 h-6 sm:w-8 sm:h-8 rounded-full border-2 {{ in_array($color, $selectedColors) ? 'border-[#8B4513] ring-2 ring-[#8B4513] ring-offset-1' : 'border-gray-300' }} 
                                    ...
                                    {{ strtolower($color) === 'navy' ? 'bg-blue-900' : '' }}
                                    {{ strtolower($color) === 'gray' ? 'bg-gray-400' : '' }}
...
<section class="py-6 sm:py-8 lg:py-12 bg-white border-t border-gray-200">
    <div class="container mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-4 sm:mb-6">
            ...
            <p class="text-xs sm:text-sm text-gray-600">Explore our men's fashion collection</p>
        </div>
