...
@section('title', 'Anarkali Kurtis - AURA KURTIS')

@section('content')

@php
// Get products from MockData - specifically anarkali kurtis
$allProducts = \App\Helpers\MockData::getHomepageProducts();
$anarkaliProducts = $allProducts['kurtis_anarkali'] ?? [];
$bestsellers = $allProducts['bestsellers'] ?? [];

// If no specific anarkali, use all products and filter
if (empty($anarkaliProducts)) {
    $anarkaliProducts = collect($allProducts)->flatten(1)->toArray();
}

// Add hover images and assign to anarkali
$productsWithHover = [];
foreach ($anarkaliProducts as $index => $product) {
    $nextIndex = ($index + 1) % count($anarkaliProducts);
    $product['hover_image'] = $anarkaliProducts[$nextIndex]['image'] ?? $product['image'];
    $product['color'] = ['Red', 'Blue', 'Black', 'Pink', 'Green', 'Yellow', 'White', 'Orange'][$index % 8] ?? 'Red';
    $product['size'] = ['S', 'M', 'L', 'XL', 'XXL'][$index % 5] ?? 'M';
    $product['category'] = 'Anarkali';
    $product['subcategory'] = 'Anarkali';
    ...
    <div class="relative container mx-auto px-4 sm:px-6 h-full flex items-center justify-center text-center">
        <div class="text-white max-w-3xl">
            ...
            <p class="mt-4 text-sm sm:text-base text-gray-200 max-w-2xl mx-auto">
                Discover elegant anarkali kurtis crafted for modern women
            </p>
        </div>
    </div>
</section>
