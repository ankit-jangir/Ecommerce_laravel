...
@php
// Get products from MockData - specifically women kurtis
$allProducts = \App\Helpers\MockData::getHomepageProducts();
$kurtisProducts = $allProducts['women_kurtis'] ?? [];
$bestsellers = $allProducts['bestsellers'] ?? [];

// If no specific kurtis, use all products and filter
if (empty($kurtisProducts)) {
    $kurtisProducts = collect($allProducts)->flatten(1)->toArray();
}

// Add hover images and assign to kurtis
$productsWithHover = [];
foreach ($kurtisProducts as $index => $product) {
    $nextIndex = ($index + 1) % count($kurtisProducts);
    $product['hover_image'] = $kurtisProducts[$nextIndex]['image'] ?? $product['image'];
    ...
    <div class="relative container mx-auto px-4 sm:px-6 h-full flex items-center justify-center text-center">
        <div class="text-white max-w-3xl">
            ...
            <p class="mt-4 text-sm sm:text-base text-gray-200 max-w-2xl mx-auto">
                Discover elegant kurtis crafted for modern women
            </p>
        </div>
    </div>
</section>
