...
@section('title', 'Men Kurtas - AURA KURTIS')

@section('content')

@php
// Get products from MockData - specifically men kurtas
$allProducts = \App\Helpers\MockData::getHomepageProducts();
$kurtaProducts = $allProducts['men_kurtas'] ?? [];
$bestsellers = $allProducts['bestsellers'] ?? [];

// If no specific kurtas, use all products and filter
if (empty($kurtaProducts)) {
    $kurtaProducts = collect($allProducts)->flatten(1)->toArray();
}

// Add hover images and assign to kurtas
$productsWithHover = [];
foreach ($kurtaProducts as $index => $product) {
    $nextIndex = ($index + 1) % count($kurtaProducts);
    $product['hover_image'] = $kurtaProducts[$nextIndex]['image'] ?? $product['image'];
    $product['color'] = ['Red', 'Blue', 'Black', 'Pink', 'Green', 'Yellow', 'White', 'Orange', 'Navy', 'Gray'][$index % 10] ?? 'Red';
    $product['size'] = ['S', 'M', 'L', 'XL', 'XXL'][$index % 5] ?? 'M';
    $product['category'] = 'Kurtas & Jackets';
    $product['subcategory'] = 'Kurtas & Jackets';
    ...
    <div class="relative container mx-auto px-4 sm:px-6 h-full flex items-center justify-center text-center">
        <div class="text-white max-w-3xl">
            ...
            <p class="mt-4 text-sm sm:text-base text-gray-200 max-w-2xl mx-auto">
                Discover premium kurtas crafted for modern men
            </p>
        </div>
    </div>
</section>
