...
@php
// Get products from MockData - specifically men pants
$allProducts = \App\Helpers\MockData::getHomepageProducts();
$pantsProducts = $allProducts['men_pants'] ?? [];
$bestsellers = $allProducts['bestsellers'] ?? [];

// If no specific pants, use all products and filter
if (empty($pantsProducts)) {
    $pantsProducts = collect($allProducts)->flatten(1)->toArray();
}

// Add hover images and assign to pants
$productsWithHover = [];
foreach ($pantsProducts as $index => $product) {
    $nextIndex = ($index + 1) % count($pantsProducts);
    $product['hover_image'] = $pantsProducts[$nextIndex]['image'] ?? $product['image'];
    ...
    <div class="relative container mx-auto px-4 sm:px-6 h-full flex items-center justify-center text-center">
        <div class="text-white max-w-3xl">
            ...
            <p class="mt-4 text-sm sm:text-base text-gray-200 max-w-2xl mx-auto">
                Discover premium pants crafted for modern men
            </p>
        </div>
    </div>
</section>
